...
# run loadCoverage()
R-devel -e "library(derfinder2); dirs <- makeBamList(datadir='${DATADIR}', samplepatt='RR'); loadCoverage(dirs=dirs, chr='${chrnum}', cutoff=5, output='auto', verbose=TRUE); proc.time(); sessionInfo()"
